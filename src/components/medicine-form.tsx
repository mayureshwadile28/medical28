    ...
    .superRefine((data, ctx) => {
      if (data.category === "Tablet" || data.category === "Capsule") {
        if (data.tablets_per_strip === undefined || data.tablets_per_strip < 1) {
          ctx.addIssue({
            code: "custom",
            message: "Tablets per strip is required and must be at least 1.",
            path: ["tablets_per_strip"],
          })
        }
        data.batches.forEach((batch, index) => {
          if (batch.stock_strips === undefined || batch.stock_strips < 0) {
            ctx.addIssue({
              code: "custom",
              message: "Strips required.",
              path: [`batches.${index}.stock_strips`],
            })
          }
        })
        ...
        data.batches.forEach((batch, index) => {
          if (
            ...
            ctx.addIssue({
              ...
              path: [`batches.${index}.stock_quantity`],
            })
          }
        })
      }
      if (
        data.category === "Other" &&
        (!data.customCategory || data.customCategory.trim().length < 2)
      ) {
        ctx.addIssue({
          code: "custom",
          message: "Please specify a category name (at least 2 characters).",
          path: ["customCategory"],
        })
      }
      ...
      const descriptionFields = [
        ...
        data.description_minAge,
        data.description_maxAge,
        data.description_gender,
      ]
      const filledDescriptionFields = descriptionFields.filter(
      ...
      if (filledDescriptionFields > 0) {
        if (!data.description_patientType) {
          ctx.addIssue({
            code: "custom",
            message: "Patient type is required if providing a description.",
            path: ["description_patientType"],
          })
        }
        if (!data.description_illness?.trim()) {
          ctx.addIssue({
            code: "custom",
            message: "Illness is required if providing a description.",
            path: ["description_illness"],
          })
        }
        if (data.description_patientType === "Human") {
          if (
            data.description_minAge === undefined ||
            data.description_minAge <= 0
          ) {
            ctx.addIssue({
              code: "custom",
              message: "Min age is required and must be > 0.",
              path: ["description_minAge"],
            })
          }
          if (
            data.description_maxAge === undefined ||
            data.description_maxAge <= 0
          ) {
            ctx.addIssue({
              code: "custom",
              message: "Max age is required and must be > 0.",
              path: ["description_maxAge"],
            })
          }
          if (!data.description_gender) {
            ctx.addIssue({
              code: "custom",
              message:
                "Gender is required if providing a description for humans.",
              path: ["description_gender"],
            })
          }
          if (
            data.description_minAge !== undefined &&
            data.description_maxAge !== undefined &&
            data.description_maxAge < data.description_minAge
          ) {
            ctx.addIssue({
              code: "custom",
...
export function MedicineForm({
    ...
    if (startWithNewBatch) {
      ...
      if (orderItem && medicineToEdit.category) {
        const isTabletCategory =
          medicineToEdit.category === "Tablet" ||
          medicineToEdit.category === "Capsule"
        const qtyValue = orderItem.quantity
          ? parseInt(orderItem.quantity.replace(/\D/g, "")) || 0
          : 0

        if (isTabletCategory) {
          const strips =
            orderItem.unitName && orderItem.unitsPerPack
              ? qtyValue * orderItem.unitsPerPack
              : qtyValue
  ...
  const handleSubmit = (values: FormData) => {
    ...
    if (
      values.description_patientType === "Human" &&
      formattedIllness &&
      values.description_minAge &&
      values.description_maxAge &&
      values.description_gender &&
      values.description_minAge > 0 &&
      values.description_maxAge > 0
    ) {
      hasFullDescription = true
    } else if (
      values.description_patientType === "Animal" &&
      formattedIllness
    ) {
      hasFullDescription = true
    }
    ...
    const newBatches: Batch[] = values.batches.map(b => {
      ...
      if (finalCategory === "Tablet" || finalCategory === "Capsule") {
        stock.tablets = Math.round(
          (b.stock_strips || 0) * (values.tablets_per_strip || 1)
        )
      } else {
    ...
    if (finalCategory === "Tablet" || finalCategory === "Capsule") {
      ;(medicineData as TabletMedicine).tabletsPerStrip =
        values.tablets_per_strip || 10
    }
         ...
         <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="space-y-4">
            ...
            {fields.map((field, index) => (
              <div
                key={field.id}
                className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-7 gap-2 p-3 border rounded-lg relative bg-muted/50"
              >
                <FormField
                  control={form.control}
          ...
          >
            ...
            <CollapsibleContent className="mt-4 space-y-4 p-4 border rounded-lg bg-muted/30">
              <div className="flex justify-between items-center mb-4">
                <FormLabel>
                  Provide usage details for smart suggestions.
                </FormLabel>
                <Button
                  type="button"
                  variant="ghost"
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <Textarea
                        placeholder="e.g., Fever, Cold, Body Pain, Vomiting, Skin Infection"
                        {...field}
                      />
                    </FormControl>
                    <FormMessage />
